...
$restrictions = " and recipient_type = 'P' and cycle between $min_cycle and $max_cycle and recipient_ext_id in (select nimsp_candidate_id from legislators union select nimsp_candidate_id from governors)";
...
dbwrite("delete from contributions_dem where transaction_id in (select * from (select transaction_id from contributions_dem a left join legislator_terms b on recipient_ext_id = imsp_candidate_id and cycle = term and recipient_state = state  where imsp_candidate_id is null and a.seat != 'state:governor') a ) or (recipient_ext_id not in (select imsp_candidate_id from legislator_terms where term = $max_cycle union select nimsp_candidate_id from governors) and cycle < $min_cycle) or recipient_type != 'P' or company_id = 1");
